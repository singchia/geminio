...
type SessionAbove interface {
    SessionID() uint64      // 获取对话ID
    SetSessionID(uint64)    // 设置对话ID
}
```
...
Dialogue.readInCh  [Dialogue receive channel]
    |
    v
Dialogue.handlePkt()  [Dialogue process packet]
    |
    v
...
DialogueMgr.writeScheduler()  [Round-robin scheduler]
    |
    +-- Poll all Dialogue.writeOutCh
    +-- Max 1 packet per Dialogue per round
    `-- Use reflect.Select for dynamic selection
    |
    v
Dialogue.handleOut()  [Process send packet]
    |
    v
Connection.Write()  [Write to connection layer]
    |
    v
Connection.writeOutCh  [Connection layer send channel]
    |
    v
Connection.writePkt()  [Encode and send to network]
    |
    v
Network Send
```
...
func (dm *dialogueMgr) writeScheduler() {
    for {
        ...
        // 2. 如果所有Dialogue都没有数据，阻塞等待
        if noDataAvailable {
            reflect.Select(cases)  // 动态构建select cases
        }
    }
}
```
...
- **轮询索引**: 使用原子操作维护轮询索引，确保每个Dialogue都有机会
- **每轮限制**: 每个Dialogue每轮最多处理一个数据包，防止某个Dialogue独占
- **动态更新**: 当Dialogue添加/删除时，调度器自动更新channel列表
...
Initiator                        Recipient
   ...
   |                               |  Route to corresponding Dialogue
```
...
在端到端通信系统中，有两个关键问题需要解决：

#### 5.1 顺序一致性 (Ordering Consistency)

**问题**: 发送的消息和接收的消息，顺序是否一致？
...
#### 5.2 消息到达语义 (Delivery Semantics)

**问题**: 消息的到达语义 - 最多一次、至少一次和准确处理一次
...
| 语义类型 | 丢失 | 重复 | 确认机制 | 去重机制 | 适用场景 |
|---------|------|------|---------|---------|---------|
| At-Most-Once | 可能 | 不会 | 无 | 无 | 日志、指标 |
| At-Least-Once | 不会 | 可能 | 有 | 无 | 一般消息 |
...
2. **批量处理**: 调度器可以批量处理多个Dialogue的数据包
3. **动态调整**: 调度器列表动态更新，适应Dialogue的添加/删除
