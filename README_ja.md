...
- ⚡ **約 5 GB/s** のストリームスループットと **約 2.3 万 RPC ラウンドトリップ/秒**（ラップトップクラス CPU で計測。[ベンチマーク](#ベンチマーク)参照）。
...
ストリーム約 3.9 万件/秒・5 GB/s、エンドツーエンドの RPC ラウンドトリップ約 2.3 万件/秒・3 GB/s、短メッセージ約 6.8 万件/秒・8.9 GB/s。自分のマシンで `make bench` をどうぞ。
